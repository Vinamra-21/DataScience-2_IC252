# 2. (a) Simulate a random number generator for the following distributions:
...
def nrm_rd(mean , std , size):  # simulate normal by Box-Muller transformation
    u1 = np.random.rand(size)
    u2 = np.random.rand(size)
    z0 = np.sqrt(-2 * np.log(u1)) * np.cos(2 * np.pi * u2)
    z1 = np.sqrt(-2 * np.log(u1)) * np.sin(2 * np.pi * u2)
    return mean + z1*std  # or return z0
